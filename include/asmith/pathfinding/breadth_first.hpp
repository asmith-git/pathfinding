...
#ifndef ASMITH_PATHFINDING_BREADTH_FIRST_HPP
#define ASMITH_PATHFINDING_BREADTH_FIRST_HPP
...
namespace asmith { namespace pathfinding {
	...
	class breadth_first : public basic_pathfinder<K,C> {
	public:
		...
		std::vector<key_t> find_path(const map<key_t, C>& aMap, const key_t aBegin, const key_t aEnd) const throw() override {
			...
			mNodes.emplace(aBegin, node({ aBegin, aBegin, true }));
			...
			while(! mStack.empty()) {
				...
				for(size_t i = 0; i < count; ++i) {
					...
					auto j = mNodes.find(k);
					if(j == mNodes.end()) {
						mNodes.emplace(k, node({ k, n.key, true }));
						mStack.push_back(k);
					}
				}
			}
